...
public class MyPlayerInput : MonoBehaviour
{
    ...
    private float moveCooldown = 0.2f; // Hareket için bekleme süresi
    private float lastMoveTime = 0f;

    private bool isBlockPlaced = false; // Yeni blok yerleşimi kontrol bayrağı

    private Vector2 lastDragPosition; // Son sürükleme pozisyonu
    ...
    {
        var current = GameManager.Instance.Current;
        while (GameManager.Instance.IsInside(current.GetPreviewPosition()))
        {
            current.Move();
        }
