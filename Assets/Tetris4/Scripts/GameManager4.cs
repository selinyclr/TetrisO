...
public class GameManager4 : MonoBehaviour
{
    ...
    public MyBlockController4 Current { get; set; }
    ...
    [SerializeField] private List<MyBlockController4> listPrefabs;

    private List<MyBlockController4> _listHistory = new List<MyBlockController4>();
    ...
    {
        bool rowDeleted = false; // Satırın temizlenip temizlenmediğini kontrol eden bayrak

        for (int i = 0; i < GridSizeY; i++)
        {
            if (IsFullRow(i))
            {
                rowDeleted = true; // Satırın temizlendiğini işaretle
        ...
        if (rowDeleted) // Eğer bir satır temizlendiyse skoru güncelle
        {
