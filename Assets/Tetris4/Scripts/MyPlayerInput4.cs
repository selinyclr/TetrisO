...
public class MyPlayerInput4 : MonoBehaviour
{
    ...
    private void SagaHareketFNC()
    {
        ...
        var value = 1;
        var isMovable = GameManager4.Instance.IsInside(GetPreviewHorizontalPosition(value));
        if (isMovable)
    ...
    private List<Vector2> GetPreviewPosition()
    {
        ...
        var listPiece = GameManager4.Instance.Current.ListPiece;
        var pivot = GameManager4.Instance.Current.transform.position;
    ...
    {
        if (Vector2.Distance(suruklemeHareket, lastDragPosition) > 1.0f) 
        {
    ...
    {
        var current = GameManager4.Instance.Current.transform;
    ...
    {
        var current = GameManager4.Instance.Current;
        while (GameManager4.Instance.IsInside(current.GetPreviewPosition()))
        {
            current.Move();
        }
        isBlockPlaced = true; // Blok yerleştirildiğinde bayrağı ayarla
    }

    public void ResetBlockPlaced()
    {
        isBlockPlaced = false; // Yeni blok oluşturulduğunda bayrağı sıfırla
    }
}
